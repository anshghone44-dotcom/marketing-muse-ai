...
export default function AiContentGeneratorChat({ companyData, onCompanySubmit }: Props) {
  ...
  const generateContent = () => {
    ...
    setTimeout(() => {
      ...
      switch (contentType) {
        case "blog":
          mockBody = `### Introduction\nIn today's fast-paced ${industry || "market"}, businesses like yours are looking for ways to optimize ${product || "their workflow"}. Whether you're a seasoned pro or just starting out, understanding the impact of ${topic} is crucial.\n\n### Why ${topic} Matters for ${audience || "Modern Professionals"}\n1. **Increased Efficiency**: Our data shows that ${name || "a specialized solution"} can reduce overhead by 30%.\n2. **Scalability**: As ${topic} evolves, your strategy must adapt to stay competitive.\n\n### Conclusion\nWrapping up, the future of ${product || "industry innovation"} lies in how we leverage ${topic} today.`;
          break;
        case "landing":
          mockBody = `# Transform Your ${industry || "Business"} with ${name || "LeadBot"}\n\n**${topic}** is no longer just an option—it's a requirement for ${audience || "success"}. Our ${tone.toLowerCase()} approach ensures you get results without the typical ${industry || "technical"} headaches.\n\n## Features\n- **Seamless Integration**: Connect with your existing tools in minutes.\n- **AI-Powered Insights**: Let our models do the heavy lifting for ${topic}.\n- **24/7 Support**: We're here whenever you need us.`;
          break;
        case "email":
          mockBody = `Subject: Quick question about your ${topic} strategy\n\nHi there,\n\nI was looking into ${name || "our latest research"} and thought of you. Many ${audience || "industry leaders"} are currently struggling with ${topic}, and I'm curious how you're approaching it at the moment.\n\nWe've been helping folks in the ${industry || "business"} space achieve better results through ${product || "our AI tools"}. Would you be open to a 5-minute chat next week?\n\nBest,\nThe ${name || "LeadBot"} Team`;
          break;
        case "social":
          mockBody = `🚀 Are you ready to level up your ${topic} game?\n\nIn the ${industry || "marketing"} world, staying ahead means embracing ${product || "new tech"}. Here's 3 reasons why ${name || "our solution"} is a game-changer for ${audience || "your team"}:\n\n✅ Automation that actually works\n✅ Data-driven insights for ${topic}\n✅ Built for growth\n\nDouble tap if you agree! 💡 #AI #${industry || "innovation"} #${topic}`;
          break;
        default:
